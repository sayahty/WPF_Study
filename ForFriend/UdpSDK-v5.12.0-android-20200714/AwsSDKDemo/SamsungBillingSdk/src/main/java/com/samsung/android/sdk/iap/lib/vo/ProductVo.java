...
//        return dump;
        ...
        try {
            jsonObject = new JSONObject(super.dump());
            jsonObject.put("mSubscriptionDurationUnit",getSubscriptionDurationUnit());
            jsonObject.put("mSubscriptionDurationMultiplier",getSubscriptionDurationMultiplier());
            jsonObject.put("mFreeTrialPeriod",getFreeTrialPeriod());
            jsonObject.put("mItemImageUrl",getItemImageUrl());
            jsonObject.put("mItemDownloadUrl",getItemDownloadUrl());
            jsonObject.put("mReserved1",getReserved1());
            jsonObject.put("mReserved2",getReserved2());
            dump = jsonObject.toString();
        } catch (JSONException e) {
            e.printStackTrace();
            dump = dump.substring(0,dump.lastIndexOf("}"))+","+
                    "mSubscriptionDurationUnit':'"+getSubscriptionDurationUnit()+"',"+
                    "'mSubscriptionDurationMultiplier':'"+getSubscriptionDurationMultiplier()+"',"+
                    "'mFreeTrialPeriod':'"+getFreeTrialPeriod()+"',"+
                    "'mItemImageUrl':'"+getItemImageUrl()+"',"+
                    "'mItemDownloadUrl':'"+getItemDownloadUrl()+"',"+
                    "'mReserved2':'"+getReserved2()+"',"+
                    "'mReserved1':'"+getReserved1()+"'"+
                    "'}";
        }
        return dump;
    }
