...
//        return dump;
        ...
        try {
            ...
            jsonObject.put("mPaymentId",getPaymentId());
            jsonObject.put("mPurchaseId",getPurchaseId());
            jsonObject.put("mPurchaseDate",getPurchaseDate());
            jsonObject.put("mPassThroughParam",getPassThroughParam());
            ...
            jsonObject.put("mVerifyUrl",getVerifyUrl());
            dump = jsonObject.toString();
        } catch (JSONException e) {
            e.printStackTrace();
            dump = dump.substring(0,dump.lastIndexOf("}"))+","+
                    "mPaymentId':'"+getPaymentId()+"',"+
                    "'mPurchaseId':'"+getPurchaseId()+"',"+
                    "'mPurchaseDate':'"+getPurchaseDate()+"',"+
                    "'mPassThroughParam':'"+getPassThroughParam()+"',"+
                    "'mItemImageUrl':'"+getItemImageUrl()+"',"+
                    "'mItemDownloadUrl':'"+getItemDownloadUrl()+"',"+
                    "'mReserved2':'"+getReserved2()+"',"+
                    "'mReserved1':'"+getReserved1()+"',"+
                    "'mVerifyUrl':'"+getVerifyUrl()+"'"+
                    "'}";
        }
        return dump;
    }
